...
class GlobalCritic(nn.Module):
    ...
    def forward_global_q(self, obs, a, global_s, global_a, global_mask):
        ...
        q1 = self.q1_fc_o(oa)
        q1_ = self.q1_fc_s(sa.reshape(sa.size(0), -1))
        q1 = self.q1_v(torch.cat([q1, q1_], -1))

        q2 = self.q2_fc_o(oa)
        q2_ = self.q2_fc_s(sa.reshape(sa.size(0), -1))
...
class AblationV3(object):
    """
    Ablation Model V1 that removes the GAT and VAEs from the centralized critic
    """
    def __init__(
            self,
            obs_dim,
...
if __name__ == '__main__':
    ...
    model = AblationV3(obs_dim=91, action_dim=2, state_dim=10, hidden_dim=128, max_action=1, max_agents=30)
