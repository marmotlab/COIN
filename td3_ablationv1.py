...
class GlobalCritic(nn.Module):
    def __init__(self, obs_dim, state_dim, action_dim, hidden_dim, vae_dim, max_agents):
        ...
        self.q1_fc_s = nn.Sequential(
            nn.Linear((state_dim + action_dim + vae_dim) * max_agents, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
        ...
        self.fc_s = nn.Sequential(
            nn.Linear((state_dim + action_dim) * max_agents, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, hidden_dim // 2),
    ...
    def forward_global_q(self, obs, a, global_s, global_a, global_mask):
        ...
        mu, logvar = self.forward_vae_encode(obs, global_s[:, 0, :], a, global_s, global_a)
        std = torch.exp(0.5 * logvar)
        eps = torch.randn_like(std)
        z = mu + eps * std

        q1 = self.q1_fc_o(oa)
        q1_ = self.q1_fc_s(torch.cat([sa, z.detach()], -1).reshape(sa.size(0), -1))
        q1 = self.q1_v(torch.cat([q1, q1_], -1))

        q2 = self.q2_fc_o(oa)
        q2_ = self.q2_fc_s(torch.cat([sa, z.detach()], -1).reshape(sa.size(0), -1))
    ...
    def V(self, obs, global_s, global_a, global_mask):
        ...
        v = self.fc_o(obs)
        v_ = self.fc_s(sa.reshape(sa.size(0), -1))
        v = self.fc_v(torch.cat([v, v_], -1).reshape(sa.size(0), -1))
        return v
...
class AblationV1(object):
    """
    Ablation Model V1 that removes the GAT from the centralized critic
    """
    def __init__(
            self,
            obs_dim,
        ...
        self.global_critic = GlobalCritic(obs_dim, state_dim, action_dim, hidden_dim, vae_dim, max_agents).to(self.device)
...
if __name__ == '__main__':
    ...
    model = AblationV1(obs_dim=91, action_dim=2, state_dim=10, hidden_dim=128, max_action=1, max_agents=30)
