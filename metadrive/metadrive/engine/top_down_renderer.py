...
def draw_top_down_trajectory(
    ...
    for frame in episode_data["frame"]:
        for k, state, in frame[TARGET_VEHICLES].items():
            ...
            if k not in agent_traj_dict.keys():
                agent_traj_dict[k] = dict(traj=[],
                                            color=color)

            pos_x, pos_y = surface.pos2pix(state["position"][0], state['position'][1])
            agent_traj_dict[k]['traj'].append((pos_x, pos_y))

    for agent, traj_and_color in agent_traj_dict.items():
        pygame.draw.lines(surface, traj_and_color['color'], False, traj_and_color['traj'], 2)
