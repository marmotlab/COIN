...
from metadrive.component.pgblock.customized_ramp import CustomizedRamp
...
MAMergingConfig = dict(
    spawn_roads=[
        ...
        Road(CustomizedRamp.node(1, 1, 0), CustomizedRamp.node(1, 1, 1)),
    ],

    num_agents=12,
    map_config=dict(exit_length=60, lane_num=2),
    top_down_camera_initial_x=140,
...
class MAMergingMap(PGMap):
    def _generate(self):
        ...
        # Build Customized Ramp
        last_block = CustomizedRamp(
            1,
...
class MAMergingSpawnManager(SpawnManager):
    ...
    def update_destination_for(self, agent_id, vehicle_config):
        # end_roads = copy.deepcopy(self.engine.global_config["spawn_roads"])B
        # for temp in end_roads:
        #     print(temp, type(temp))
        #     print(temp.start_node, type(temp.start_node))
        end_roads = [
            # Road(FirstPGBlock.NODE_2, FirstPGBlock.NODE_3),
            Road(CustomizedRamp.node(1, 3, 0), CustomizedRamp.node(1, 3, 1)),
            Road(CustomizedRamp.node(1, 4, 3), CustomizedRamp.node(1, 4, 3)),
            Road(CustomizedRamp.node(1, 4, 2), CustomizedRamp.node(1, 4, 3)),
...
def _vis():
    ...
    for i in range(1, 100000):
        ...
        for agent in list(env.agents.keys()):
            # if not _print and _update:
            #     _current_lane = env.agents[agent].navigation.current_lane.id
            #     _print = True
            #     _update = False
            # elif _current_lane != env.agents[agent].navigation.current_lane.id:
            #     _update = True
            # else:
            #     _print = False
            #     _update = False

            # if _print and env.agents[agent].navigation.next_ref_lanes is not None:
            #     print("======= 分隔符 =======")
            #     print('current lane:', env.agents[agent].navigation.current_lane.id)
            #     print('current ref lane:', env.agents[agent].navigation.current_ref_lanes[0].id)
            #     print('next ref lane:', env.agents[agent].navigation.next_ref_lanes[0].id)

            if not tm[agent]:
                count_1 += 1
            if count_1 == 1:
                print(env.agents[agent].navigation.checkpoints)
...
def _vis_top_down():
    ...
    for i in range(1, 100000):
        ...
        if len(env.agents) == 1:
            actions = {k: IDMPolicy(v, random_seed=1).act() for k, v in env.agents.items()}
        o, r, tm, tc, info = env.step(actions)
        env.render(mode="top_down", num_stack=25, camera_position=(145, 5), scaling=2, screen_record=False, window=True)
