...
class MultiAgentMetaDrive(MetaDriveEnv):
    ...
    def step(self, actions):
        ...
        for agent, info in i.items():
            ...
            info['spawn_lane_index']=self.agent_manager.active_agents[agent].config.spawn_lane_index
        ...
        if new_obs_dict:
            for new_id, new_obs in new_obs_dict.items():
                ...
                i[new_id]['spawn_lane_index'] = self.agent_manager.active_agents[new_id].config.spawn_lane_index
                tm[new_id] = False
                tc[new_id] = False
