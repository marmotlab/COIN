...
class GlobalCritic(nn.Module):
    def __init__(self, obs_dim, state_dim, action_dim, hidden_dim, vae_dim):
        ...
        self.q1_fc_s = nn.Sequential(
            nn.Linear((state_dim + action_dim) + vae_dim, hidden_dim // 2),
            nn.ReLU(),
    ...
    def forward_global_q(self, obs, a, global_s, global_a, global_mask):
        ...
        q1 = self.q1_fc_o(oa)
        q1_ = self.q1_fc_s(torch.cat([sa, z.detach()], -1))
        q1_ = self.q1_gat(q1_, global_mask)
        q1_ = q1_[:, 0, :]
        q1 = self.q1_v(torch.cat([q1, q1_], -1))

        q2 = self.q2_fc_o(oa)
        q2_ = self.q2_fc_s(torch.cat([sa, z.detach()], -1))
        ...
        for file_name in os.listdir(filename):
            if "best_model" in file_name:
                if "actor" in file_name:
                    ...
                    if val>=success_high:
                        success_high=val
                        best_filename=os.path.join(filename,file_name.replace("_actor",""))
                        
        print(best_filename)
        self.local_critic.load_state_dict(torch.load(best_filename + "_local_critic"))
        self.local_critic_target = copy.deepcopy(self.local_critic)

        self.global_critic.load_state_dict(torch.load(best_filename + "_global_critic"))
        self.global_critic_target = copy.deepcopy(self.global_critic)

        self.actor.load_state_dict(torch.load(best_filename + "_actor"))
        self.actor_target = copy.deepcopy(self.actor)
...
if __name__ == '__main__':
    ...
    model = TD3(obs_dim=91, action_dim=2, state_dim=10, hidden_dim=128, max_action=1)
